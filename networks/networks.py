...
class MultiScaleDis(nn.Module):
    ...
    def _classification_loss(self, logit, target, dataset='CelebA'):
        ...
        if dataset in ['CelebA']:
            return F.binary_cross_entropy_with_logits(logit, target, reduction='elementwise_mean')
        else:
    ...
    def calc_dis_loss(self, input_fake, input_real, fake_cls, real_cls, weight_gan=1.0, weight_cls=1.0):
    ...
    def calc_gen_loss(self, input_fake, target_cls, weight_gan=1.0, weight_cls=1.0):
        ...
        for it, (out0) in enumerate(outs0):
            out_src_fake, out_cls_fake = out0
...
class AdaINGen(nn.Module):
    ...
    def __init__(self, input_dim, vocab, params, pretrained_embed=None):
        ...
        # text encoder
        self.enc_txt = TxtEncoder(vocab, embed_dim, hidden_size, c_dim, num_cls, num_layers, 
            dropout_in, dropout_out, pretrained_embed=pretrained_embed)

        # MLP to generate AdaIN parameters
        self.mlp = MLP(style_dim, self.get_num_adain_params(self.dec), mlp_dim, 3, norm='none', activ=activ) # R: What is this doing?
    ...
    def decode(self, content, style):
        ...
        adain_params = self.mlp(style) # R: What is this doing?
        self.assign_adain_params(adain_params, self.dec)
        images = self.dec(content)
        return images

    def assign_adain_params(self, adain_params, model): # R: What is this doing?
    ...
    def get_num_adain_params(self, model):
        ...
        print("Num adain params: " + str(num_adain_params))
