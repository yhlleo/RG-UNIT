...
def create_triplets_labels(data_iter):
    ...
    # rnd_label: Random (negative) domain
    i_anc, i_neg, src_label, trg_label, rnd_label = data_iter

    # Data to GPU
    i_anc = i_anc.to(device)
    i_neg = i_neg.to(device)
    # should use assign_lable
    ...
    rnd_label = asign_label(rnd_label, num_class, mode=config['dataset']).to(device)
    ...
    rnd_style = dist_sampling_split(rnd_label, config['c_dim'], config['stddev'], device)
    ...
    if config['triplets']['hard'] == 1:
    # 3. Hard: A [Ec(i_a), A_t]  ;  P [Ec(T_t(I_a)), Ea(T_t(I_a))]  ; N  [Ec(T_n(i_a)), Ea(T_n(i_a))]
        # Translate anchor image to a random domain and use it as negative
        i_anc_trans_rnd = gmm_gen.gen_decode(i_anc, i_anc_con, rnd_style) 
        i_anc_trans_rnd_con, i_anc_trans_rnd_att = gmm_gen.gen_encode(i_anc_trans_rnd) 
        triplets.append([i_anc_con, trg_style, 
                         i_anc_trans_trg_con, i_anc_trans_trg_att, 
                         i_anc_trans_rnd_con, i_anc_trans_rnd_att])
