...
def asign_label(label, num_cls, mode='CelebA', normalize=True):
    if mode in ['CelebA', 'CUB200']:
        asigned_label = label.clone()
    else: # mode in ['RaFD', 'A2B']
        asigned_label = label2onehot(label, num_cls)
    if normalize:
...
def sampling_mean(z, c_dim):
    v_dim = z.size(1)//c_dim
    mu = torch.zeros((z.size(0), v_dim))
    for i in range(v_dim):
        mu[:, i] = z[:,i*c_dim:(i+1)*c_dim].mean(dim=1)
    return mu
