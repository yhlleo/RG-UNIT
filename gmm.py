...
def gmm_kl_distance(pred_mu, pred_sigma, mus, sigma=None, device=None):
...
def gmm_kl_distance_sp(pred_mus, pred_sigma, mus, sigma=None, device=None):
    """Pytorch implementation of the Kullback–Leibler divergence.
      :param: pred_mu, extracted attribute vector with shape [N, d]
      :param: mus, mean tensor with shape [N, d]
    """
...
def gmm_earth_mover_distance(pred_mu, mus, device=None):
    """Pytorch implementation of the Earth Mover.
    :param: pred, extracted attribute vector with shape [N, d*V]
    :param: mus, mean tensor with shape [N, d]
    """
    return torch.abs(pred_mu - mus).sum(dim=1).mean() 


def gmm_earth_mover_distance_sp(pred_mu, mus, device=None):
    """Pytorch implementation of the Earth Mover.
    :param: pred, extracted attribute vector with shape [N, d*V]
    :param: mus, mean tensor with shape [N, d]
    """
